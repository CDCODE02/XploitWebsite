        ...
        <div className="hidden md:flex items-center gap-8">
          {NAVIGATION.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-sm font-medium text-gray-400 hover:text-brand transition-colors tracking-wide"
            >
              {item.name}
            </a>
          ))}
        ...
        <div className="md:hidden absolute top-full left-0 w-full bg-dark-bg/95 backdrop-blur-xl border-b border-dark-border p-6 flex flex-col gap-4 animate-in fade-in slide-in-from-top-5">
          {NAVIGATION.map((item) => (
            <a
              ...
              className="text-lg font-medium text-gray-300 hover:text-brand py-2 border-b border-white/5"
            >
              {item.name}
            </a>
          ))}
